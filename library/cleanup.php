...
function timberpress_start_cleanup() {
	...
	add_action( 'init', 'timberpress_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'timberpress_remove_rss_version' );

	// Remove pesky injected css for recent comments widget
	add_filter( 'wp_head', 'timberpress_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in head
	add_action( 'wp_head', 'timberpress_remove_recent_comments_style', 1 );

	// Remove inline widget attribute from figure tag
	add_filter( 'img_caption_shortcode', 'timberpress_remove_figure_inline_style', 10, 3 );

}
add_action( 'after_theme_setup', 'timberpress_start_cleanup' );
endif;
